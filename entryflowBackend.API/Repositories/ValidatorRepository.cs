...
public class ValidatorRepository(EntryflowDbContext context) : IValidatorRepository
{
    ...
    {
        var validator = await context.Validators.SingleOrDefaultAsync(x => x.Id == id);
    ...
    {
        return await context.Validators.ToListAsync();
    }

    public async Task AddValidatorAsync(Validator validator)
    {
        await context.Validators.AddAsync(validator ?? throw new ArgumentNullException(nameof(validator)));
    }

    public void UpdateValidator(Validator validator)
    {
        context.Validators.Update(validator);
    }

    public void DeleteValidator(Validator validator)
    {
        context.Validators.Remove(validator);
    }
